...
import { getDeployLogs, formatJobLogs, getDeployHistory, formatDeployHistory, runRollback, formatRollbackInfo } from '../use-cases/deploy-observability';
...
export async function handleHistory(parsed: ParsedCommandArgs): Promise<number> {
  const repository = await resolveRequiredString(
    parsed.positionals[1] ?? getStringFlag(parsed, 'repository'), 'Repository (owner/repo)');
  ...
  if (useJson) { printJson(jobs); } else { process.stdout.write(formatDeployHistory(jobs, repository)); }
  return 0;
}
...
export async function handleLogs(parsed: ParsedCommandArgs): Promise<number> {
  ...
  if (!job) { process.stdout.write(`\n  No deploy found for ${repository}\n\n`); return 0; }
  if (useJson) { printJson(job); } else { process.stdout.write(formatJobLogs(job)); }
  return 0;
}
...
export async function handleRollback(parsed: ParsedCommandArgs): Promise<number> {
  ...
  const useJson = getBooleanFlag(parsed, 'json');
  if (useJson) { printJson(result); } else {
